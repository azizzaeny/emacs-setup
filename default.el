...
(defun open-note ()
  ;; notes should using all the script.sh or we evaluated scirpt on the fly
  ;; we gonna fix the notes
  (interactive)
  (let ((out-dir (format "%s-%s.md" "notes" (format-time-string "%Y%m%d" (current-time)))))
	(if (file-exists-p (expand-file-name (concat "~/daily-notes" out-dir)))
		(find-file out-dir)
	  (find-file out-dir)
...
(global-set-key (kbd "C-c c o") 'open-note)
